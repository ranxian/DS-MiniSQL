...
接口：
1.filePtr全局变量
2.DB类中的table operator、start和end函数
3.table类中的filePtr operator、filePtr getCataPoint、filePtr getIdxPoint、unsigned long getPageNum函数
4.memWrite函数
...
public:
    ...
    filePtr getIdxPoint() const;                                             //取得B+模块在文件中可写的首地址
...
filePtr memWrite(const void *, size_t, filePtr *);      //内存拷贝函数，返回下一个可写的filePtr
